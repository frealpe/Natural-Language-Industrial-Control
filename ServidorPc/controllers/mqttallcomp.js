    ...
    if (tipo === "Plc") {
      if (!Array.isArray(resultado) || resultado.length === 0) {
        ...
        const datosNormalizados = resultado.map(
          ({ topic, mensaje: msgPLC, orden, red, secuencia, usarIA }) => {
            let payload;
            ...
            // ... (Comparación logic stays here) ...

            // 🔹 Normalización para Comparación PLC (restored context)
            if (topic === "Plc/Comparacion") {
               ...
               payload.mensaje = msgPLC || "Solicitud de comparación.";
               if (!payload.secuencia || payload.secuencia.length === 0) {
                 payload.secuencia = [{ porcentaje: 50, duracion: 10 }];
               }
               payload.secuencia = payload.secuencia.map((s) => ({
                 porcentaje: s.porcentaje,
                 duracion_s: s.duracion ?? s.duracion_s,
               }));
            }

            // 🔹 Normalización para red Petri (restored context)
            if (topic === "Plc/Petri") {
                if (typeof payload !== "object" || payload === null) payload = {};
            ...
            if (topic === "Plc/Identificacion") {
              // Si es string, lo encapsulamos en objeto
              if (typeof payload === "string") {
                payload = { consulta: payload };
              } 
              // Si no es objeto (y no era string), forzamos objeto vacío
              else if (typeof payload !== "object" || payload === null) {
                payload = {};
              }

              // Aplicamos propiedades comunes
              payload.orden = orden ?? 1;
              
              // 🧠 Inyección explícita del flag IA
              // Si gtpServices detectó IA, lo imponemos aquí.
              if (usarIA !== undefined) {
                  payload.usarIA = usarIA;
              }
            }

            if (!topic) {
              console.warn("⚠️ Tópico no definido en respuesta IA:", { msgPLC, orden, red, secuencia });
            } else {
